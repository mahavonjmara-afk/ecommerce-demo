...
<div class="min-h-screen flex items-center justify-center py-8 bg-gradient-to-br from-[#1E40AF] via-[#4C1D95] to-[#1E3A8A] relative overflow-hidden">
    ...
    <!-- Éléments décoratifs -->
    <div class="absolute top-10 right-10 w-72 h-72 bg-amber-500/20 rounded-full blur-3xl"></div>
    <div class="absolute bottom-10 left-10 w-64 h-64 bg-violet-500/30 rounded-full blur-3xl"></div>

    <div class="w-full max-w-md px-4 relative z-10" data-aos="zoom-in">
        <div class="text-center mb-8">
            <h1 class="text-3xl font-bold text-white mb-2 drop-shadow-lg">Créer un compte ✨</h1>
            <p class="text-sm text-amber-200/90 font-medium tracking-wide">Rejoignez l'univers premium</p>
        </div>

        <!-- Carte Formulaire : EFFET VERRE PREMIUM (Glassmorphism) -->
        <div class="bg-white/10 backdrop-blur-xl border border-white/20 shadow-2xl rounded-2xl p-8" x-data="{ showPassword: false, showConfirm: false }">
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                ...
                <div>
                    ...
                    @error('name') <p class="text-xs text-rose-400 mt-2">{{ $message }}</p> @enderror
                </div>
                ...
                <!-- CGU -->
                <label class="flex items-start gap-3 text-sm cursor-pointer select-none">
                    <input type="checkbox" name="terms" required class="mt-0.5 rounded border-gray-500 text-amber-500 focus:ring-amber-500 bg-white/10">
                    <span class="text-gray-400 leading-tight">
                        J'accepte les <a href="/cgv" class="text-amber-400 hover:underline">CGU</a> et la <a href="/confidentialite" class="text-amber-400 hover:underline">politique de confidentialité</a>
                    </span>
                </label>

                <!-- Bouton Inscription (Doré Premium) -->
            ...
            <p class="text-center text-sm text-gray-400 mt-8">
                Déjà un compte ? 
                <a href="{{ route('login') }}" class="text-amber-400 font-semibold hover:text-amber-300 transition underline decoration-amber-400/30 underline-offset-4">Se connecter</a>
            </p>
        </div>
